...
AWS_PROFILE=cool-images-ec2amicreate-freeipa-server-packer packer build --timestamp-ui -var build_bucket=installers-bucket -var release_tag=$(./bump_version.sh show) -var is_prerelease=true src/packer.pkr.hcl
```
...
```console
AWS_PROFILE=cool-images-ec2amicreate-freeipa-server-packer packer build --timestamp-ui -var-file release.pkrvars.hcl src/packer.pkr.hcl
```
